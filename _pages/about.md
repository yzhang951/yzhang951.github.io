...
function updateStats() {
  const endpoints = {
    citations: "https://cdn.jsdelivr.net/gh/yzhang951/yzhang951.github.io@google-scholar-stats/gs_data_shieldsio1.json",
    num_pub: "https://cdn.jsdelivr.net/gh/yzhang951/yzhang951.github.io@google-scholar-stats/gs_data_shieldsio2.json"
  };

  fetch(endpoints.citations)
    .then(response => response.json())
    .then(data => {
      document.getElementById('citations').textContent = data.message;
    });

  fetch(endpoints.num_pub)
    .then(response => response.json())
    .then(data => {
      document.getElementById('num_pub').textContent = data.message;
    });
}
...
Our team specializes in simulating the mechanical properties of advanced engineering and energy materials, spanning from the nano to macro-scale. Our primary focus lies in materials modeling and simulation techniques. We have published <a href='https://yzhang951.github.io/publications/' target="_blank"><span id="num_pub">Loading...</span></a> papers, with <a href='https://scholar.google.com/citations?user=23XDhOwAAAAJ' target="_blank"><span id="citations">Loading...</span></a> citations, at the top international journals such as Nature, Science, Nature Communications, Science Advances, Acta Materialia and Journal of the Mechanics and Physics of Solids.
